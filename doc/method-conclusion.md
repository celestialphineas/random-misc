...
| #    | 分类学类数         | 总组数               | 序列比较                               | 聚类结果组数 |
| ---- | ------------------ | -------------------- | -------------------------------------- | ------------ |
| 1    | 31（目与梭菌各科） | 21914（即细胞数）    | 按类原始测序数据.fastq，$k=21,s=500$   | 7983         |
| 2    | 13（目）           | 7983（最大类有1920） | 按类的逐组组装序列.fasta，$k=51,s=500$ | 2839         |
| 3    | 1                  | 2839                 | 逐组协同组装的序列.fasta，$k=51,s=500$ | 974          |
| 4    | 1                  | 974                  | 逐组协同组装的序列.fasta，$k=51,s=500$ | 397          |
...
这里我们所采取的策略是细分高污染组装所对应的细胞组，试图让来自同一物种的细胞重新被分到同一组。我们利用组装中contig在原测序数据中的测序深度（depth，即原测序数据对齐到该contig上时，各处所对应的读数。部分文献将此称作coverage）作为特征，这一信息称作contig在样品中的丰度（abundance）。该特征被主流的宏基因组contig装箱工具所采用，但其目标是将contig聚类，并将宏基因组组装细分 {{Kang 2015}}。而我们的目标则是利用这个信息对细胞（即对应于宏基因组研究中的样品）聚类。
